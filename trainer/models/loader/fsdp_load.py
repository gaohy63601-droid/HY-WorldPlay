...
def maybe_load_fsdp_model(
    ...
    with set_default_dtype(param_dtype):
        ...
        if flowmap_training:
            ...
            if getattr(model, "time_r_in", None) is None:
                logger.info("Initializing flow-map r-timestep embedding before FSDP")
                model.time_r_in = TimestepEmbedder(
                    model.hidden_size,
                    get_activation_layer("silu"),
                    dtype=next(model.parameters()).dtype,
                    device=next(model.parameters()).device,
                )
                model.time_r_in.load_state_dict(model.time_in.state_dict())
            model.flowmap_gate_value = flowmap_gate_value
            if hasattr(model, "register_to_config"):
                model.register_to_config(use_meanflow=True)
