...
### GameFactory / Minecraft preprocessing

This checkout includes the GameFactory reference preprocessing scripts from PR #47 under `datasets/hy_preprocess/`. The raw dataset root should be arranged as:

```text
datasets/gamefactory_raw/
├── annotation.csv
├── metadata/
│   └── <video_stem>.json
└── video/
...
`annotation.csv` is expected to contain `original video name`, `start frame index`, `end frame index`, and `prompt`. The metadata JSON should contain per-frame GameFactory action data; the preprocessing script converts it into dense pose/action JSONs, VAE latents, prompt embeddings, first-frame i2v conditioning, and a `dataset_index.json` consumed by `CameraJsonWMemDataset`.

Local default command:

```bash
cd /workspace/WorldPolicy/HY-WorldPlay
DATA_ROOT=/workspace/WorldPolicy/HY-WorldPlay/datasets/gamefactory_raw \
OUTPUT_DIR=/workspace/WorldPolicy/HY-WorldPlay/datasets/preprocessed_gamefactory_f129 \
TARGET_NUM_FRAMES=129 \
bash datasets/hy_preprocess/scripts/preprocess_gamefactory.sh
```

For a quick smoke test, set `NUM_SAMPLES=1`. The training script defaults to:

```text
/workspace/WorldPolicy/HY-WorldPlay/datasets/preprocessed_gamefactory_f129/dataset_index.json
```

Classifier-free guidance also needs cached negative prompt embeddings:

```bash
cd /workspace/WorldPolicy/HY-WorldPlay
bash datasets/hy_preprocess/scripts/generate_neg_prompt_pt.sh
```

This writes `hunyuan_neg_prompt.pt` and `hunyuan_neg_byt5_prompt.pt` to `local_models/` by default.
...
The flow-map causal student path is kept separate from the ordinary AR baseline:
...
| `--flowmap_training` | bool | No | False | Enable flow-map causal student training |
| `--flowmap_gate_value` | float | No | 0.25 | Gate applied to the r-timestep embedding |
| `--flowmap_deltatime_type` | str | No | r | Flow-map delta-time conditioning type |
| `--flowmap_diffusion_ratio` | float | No | 0.5 | Ratio of samples using r=t |
| `--flowmap_consistency_ratio` | float | No | 0.25 | Ratio of samples using r=0 |
| `--flowmap_epsilon` | float | No | 5.0 | Central-difference epsilon in scheduler steps |
