...
def create_arrow_keyboard(actions, key_size=70, key_spacing=6, corner_radius=14):
    """Create joystick-style rotation overlay."""
    size = 2 * key_size + key_spacing
    img = Image.new("RGBA", (size, size), (0, 0, 0, 0))
    draw = ImageDraw.Draw(img)

    center = size // 2
    outer_radius = size // 2 - 4
    inner_radius = key_size // 4
    knob_radius = key_size // 3
    arrow_color = (255, 255, 255, 210)
    ring_color = (0, 0, 0, 128)
    knob_color = (30, 120, 255, 235)
    ...
    tri = key_size // 9
    draw.polygon(
        [(center, 14), (center - tri, 14 + tri), (center + tri, 14 + tri)],
        fill=arrow_color,
    )
    draw.polygon(
        [(center, size - 14), (center - tri, size - 14 - tri), (center + tri, size - 14 - tri)],
        fill=arrow_color,
    )
    draw.polygon(
        [(14, center), (14 + tri, center - tri), (14 + tri, center + tri)],
        fill=arrow_color,
    )
    draw.polygon(
        [(size - 14, center), (size - 14 - tri, center - tri), (size - 14 - tri, center + tri)],
        fill=arrow_color,
    )

    yaw_val = max(-1, min(1, actions.get("yaw", 0)))
    pitch_val = max(-1, min(1, actions.get("pitch", 0)))
    travel = outer_radius - knob_radius - 12
    knob_x = center + int(yaw_val * travel)
    knob_y = center - int(pitch_val * travel)
